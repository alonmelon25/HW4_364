...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/proaaronHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
tags = db.Table('tags', db.Column('searchterm_id', db.Integer, db.ForeignKey('searchterms.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
...
user_collection = db.Table('user_collections', db.Column('user_id',db.Integer, db.ForeignKey('personalgifcollections.id')),db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')))
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    if collection:
        ...
        collection = PersonalGifCollection(name=name, user_id=current_user.id, gifs=[])
        for g in gif_list:
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        objects = [get_gif_by_id(int(id)) for id in gif_picks]
        get_or_create_collection(current_user=current_user, name=form.name.data, gif_list=objects)
...
def collections():
    ...
    collections = PersonalGifCollection.query.filter_by(user_id=current_user.id)
